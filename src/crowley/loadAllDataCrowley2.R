...
loadDataCrowley$getCrowleyMicroData <- function()
{
    ...
    crowley.cov.data = loadDataCrowley$read.full.cov()
    
    out = list(exp.mat = crowley.exp.mat, cov.data = crowley.cov.data)
    return(out)
}
...
{
    ##TODO: property
    cov.data = fread(prop$crowley$data, "submission_bundle/Metadata/metadata.csv"))
    ...
    ## colz$direction = factor(colz$direction)
    ...
    cov.data = colz
    ##cov.data$cross = factor(cov.data$cross, levels = c("FG", "GH", "FH", "FF","GG","HH"))
...
##    crowley = loadDataCrowley$getCrowleyMicroData()
loadAllData$createAllInputs <- function()
{

    pracma::tic()

    recomputeProbeData = F ##prop$mnp$recomputeProbeInfo
    ...
    if(!recomputeProbeData)
    {
        ...
        probedGenes = suppressWarnings(fread(input = fp(computedProbeDataDir, "probedGenes.csv"), showProgress = F))
        probesetInfo = suppressWarnings(fread(input = fp(computedProbeDataDir, "probesetInfo.csv"), showProgress = F))
    } else {
