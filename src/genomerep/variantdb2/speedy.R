                                                ...
                                                rebuildCC       = prop$variantdb[[dbtype]]$rebuildCC)                                                
{
    ...
    if(rebuildFounder)
    {
        ...
        buildVariantDb$updateFounderVariantIDs(db)
    }
...
buildVariantDb$updateFounderVariantIDs <- function(db)
{

    chrsToCheck =  db$genotype$getChrs()
    maxv = unlist(db$genotype$iterate( parseFunc = function(strain,chr, df){max(df$variant_id)},
                                      strains = "C57BL6J", chrs = chrsToCheck, select = "variant_id"))
    ...
    db$genotype$iterate(parseFunc = helper.func, accum = accum)
}
                                                 ...
                                                 rebuildVCF = T)
{
    ...
    for(astrain in levels(founder.genotypes$strain))
    {
        ...
        db$genotype$write(astrain, chr, founder.genotypes[strain==astrain]) 
    }
                       ...
                       
{
    ...
    buildAll_for_cc <- function(db, probFile, chr, gt.founder.sampling, genez.df, var.df)
    {
        pracma::tic()
    ...
    gt.founder.sampling = getFounderSamplingTable(chr)
    ...
    accum = getIterAccumulator(batchSize=10, mem.gb = 30)
    accum$init(func = buildAll_for_cc,
               otherGlobals = list(db = db,
                                   chr = chr,
                                   gt.founder.sampling = gt.founder.sampling,
                                   genez.df= genez.df,
                                   var.df = var.df))
    ...
    for(i in 1:cc_limit)
    {
        ...
        print(probFile)
        accum$addCall(list(probFile = probFile))
    }
    accum$runAll()
    ...
    for(founder in founders)
    {
        ...
        db$diplotype$write(founder, chr, dp.founder, zipped = T)
        ...
        db$diplotypeSampling$write(strain = strainName, chr = chr, df = dp.founder.sampling, zipped = T)
    }
