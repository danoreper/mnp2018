...
micro.analysis$runallPermsFreed <- function(inp)
{
    ...
    covariateModels = c(##formulaWrapper$removeEffect("Strain:Diet", total.model),
...
micro.analysis$getPermResidParser <- function()
{
    ...
    parser$parse <- function(fit)
    {
        if(is.null(fit$fit))
        {
            ...
            y.mu = fit$y.transformed - epsilon

            ## Y0 = y.mu + epsilon
            ## ys = list()
            ## for(i in 1:10)
            ## {
            ##     eps = epsilon[sample(1:length(epsilon), replace = F)]
            ##     Y = y.mu + eps

            ##     z = (((lmerTest::lmer(Y ~ 1 + sv.1 + sv.2 + sv.3 + sv.4 + sv.5 + sv.6 + sv.7 + Batch + Pipeline + Diet + Strain + (1|Dam.ID), data = fit$fit$data))))
            ##   ##  browser()
            ##     print(summary(z))
            ## ##    print(ranef(z))
            ## }
        ...
        return(list(y.mu = y.mu, epsilon = epsilon, inversionParams=inversionParams))
    }
...
##    reducedModelString = covariateModelString
    ...
    for(shuffleIndex in 1:numPerm)
    {
        ...
        y.shuffled             = toperm$y.mu + toperm$epsilon[shuffle,]
